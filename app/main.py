...
options = {'hostname': os.getenv('LOGGING_HOSTNAME'), 'index_meta': True}
handler = LogDNAHandler(key, options)
log.addHandler(handler)


def exception_log_handler(func):
...
def create_new_user():
    ...
    log.info('{} timing: {}'.format(str(request.path), str(end_ts - start_ts)))
...
def derive_saved_actions(guid):
    ...
    sql = """ SELECT
        ...
        reward
    FROM action a INNER JOIN organization o on o.organizationId = a.organizationId
        INNER JOIN (SELECT actionId, updateDT from userAction WHERE userGuid = '{}' AND status = 'SAVED') ua
            on ua.actionId = a.actionId
        ORDER BY ua.updateDT DESC
...
def derive_completed_actions(guid):
    ...
    sql = """ SELECT
        ...
        o.description as 'organizationDescription',
        o.url as 'organizationUrl',
        'completed' as 'sourceList',
        reward
    FROM action a INNER JOIN organization o on o.organizationId = a.organizationId
        INNER JOIN (SELECT actionId, updateDT from userAction WHERE userGuid = '{}' AND status = 'COMPLETE') ua
            on ua.actionId = a.actionId
        ORDER BY ua.updateDT DESC
        LIMIT 20;""".format(guid)
    return query_from_sql(sql)
...
def derive_new_actions_by_key(query_key, guid):
    ...
    if query_key == 'Projects':
        where_clause = """AND a.actionType IN ('Petition', 'Write Letters')"""
    if query_key == 'Engagements':
        where_clause = """AND a.actionType IN ('Petition', 'Write Letters')"""
    if query_key == 'Criminal Justice Reform':
